...
def index():
    """FIXME: Make this the documentation for the API.
    """
    return render_template('index.html')


@app.route('/api/v1/compile', methods=['POST'])
def POST_api_v1_compile():
...
@app.route('/api/v1/compile/<string:job_id>', methods=['GET'])
def POST_api_v1_compile_job_id(job_id):
...
@app.route('/api/v1/compile/<string:job_id>/hex', methods=['GET'])
def POST_api_v1_compile_job_id_hex(job_id):
...
@app.route('/api/v1/compile/<string:job_id>/src', methods=['GET'])
def POST_api_v1_compile_job_id_src(job_id):
